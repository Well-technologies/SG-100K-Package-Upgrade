...
@Inject constructor(
    ...
    var sync: LiveData<Resource<ResourceData<CommonResponce>>>? = Transformations
        .switchMap(_testId) { input ->
            input.ifExists { participantRequest, spirometryRecordList, comment, device_id, turbine_id , nuvoairLauncherMeasurement, contraindication ->
    ...
    fun setData(
        ...
        turbine_id: String?,
        nuvoairLauncherMeasurement: NuvoairLauncherMeasurement?,
        contraindication: List<Map<String, String>>?
    ) {
        val update = TestId(participant, recordList, comment, device_id, turbine_id, nuvoairLauncherMeasurement, contraindication)
    ...
    var bodyMeasurementMeta: LiveData<Resource<ResourceData<StationData<BodyMeasurementMeta>>>>? = Transformations
        .switchMap(_bodyMeasurementMetaId) { input ->
            input.ifExists { participant, isOnline ->
                bodyMeasurementRequestRepository.getBodyMeasurementMetaNew(participant, isOnline)
    ...
    data class TestId(
        ...
        val contraindication: List<Map<String, String>>?
    ) {
        fun <T> ifExists(f: (ParticipantRequest, ArrayList<SpirometryRecord>, String?, String?, String?, NuvoairLauncherMeasurement?, List<Map<String, String>>? ) -> LiveData<T>): LiveData<T> {
            return if (participant == null || recordList == null) {
                AbsentLiveData.create()
            } else {
                f(participant!!, recordList!!, comment, device_id, turbine_id, nuvoairLauncherMeasurement, contraindication)
            }
        }
    }

    // partial measurement able -----------------------------------------------------------------------

    var hasPartiallyAble: MutableLiveData<Boolean> = MutableLiveData<Boolean>().apply { }

    var isChecked: Boolean = false

    fun setPartiallyAble(explained: Boolean) {
        isChecked = explained
        hasPartiallyAble.value = (explained)
    }
