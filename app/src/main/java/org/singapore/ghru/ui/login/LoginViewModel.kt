...
//    new login flow with auth0

    private val _loginIdNew: MutableLiveData<LoginIdNew> = MutableLiveData()

    data class LoginIdNew(val email: String?) {
    ...
    fun setLoginNew(email: String?, online : Boolean) {
        isOnline = online
        val update = LoginIdNew(email)
        if (_loginIdNew.value == update) {
            return
        }
        _loginIdNew.value = update
    }

    var accessTokenNew: LiveData<Resource<AccessToken>>? = Transformations
        .switchMap(_loginIdNew) { input ->
            input.ifExists { email ->
                accessTokenRepository.loginUserNew(email,isOnline)
            }
        }
}
