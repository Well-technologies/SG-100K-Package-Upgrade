package org.singapore.ghru.ui.questionnaire.scanbarcode.manualentry
...
import org.singapore.ghru.HLQSelfTwoActivity
import org.singapore.ghru.R
import org.singapore.ghru.WebViewTwoActivity
...
import org.singapore.ghru.ui.questionnaire.scanbarcode.ScanBarcodeViewModel
...
//                        .navigate(R.id.action_ManualEntryBarcodeFragment_to_WebFragment, bundle)
                    val partiJson: String = MemberTypeConverters.gson.toJson(participantRequest)
                    val queJson: String = MemberTypeConverters.gson.toJson(questionnaire)
                    Log.d("SELECTED_PARTICIPANT" , " IS: " + participantRequest)
                    prefs?.edit()?.putString("ParticipantRequest", partiJson)?.apply()
                    prefs?.edit()?.putString("Questionnaire", queJson)?.apply()
                    val intent = Intent(activity, WebViewTwoActivity::class.java)
...
//        })
        ...
        scanbarcodeViewModel.checkoutParticipant.observe(this, Observer { participantResource ->
        ...
        if (!checkSum.error) {
            activity?.runOnUiThread({
                ...
                if (isNetworkAvailable()) {
                    //scanbarcodeViewModel.setScreeningId(binding.editTextCode.text.toString())
                    scanbarcodeViewModel.setCheckoutScreeningId(binding.editTextCode.text.toString())
                } else {
