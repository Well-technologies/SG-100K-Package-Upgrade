package org.singapore.ghru.ui.foodquestionnaire.manualentry
...
class ManualEntryFoodFrequencyFragment : Fragment(), Injectable {
    ...
    @Inject
    lateinit var viewModel: ManualEntryFoodFrequencyViewModel
        ...
        disposables.add(
            StationCheckRxBus.getInstance().toObservable()
                .subscribe({ result ->
                    ...
                    findNavController().navigate(R.id.action_manualScanFragment_to_languageFragment, bundle)
                }, { error ->
                    print(error)
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        viewModel.participant.observe(this, Observer { participantResource ->
            ...
            if (participantResource?.status == Status.SUCCESS) {
                ...
                val bundle = bundleOf("ParticipantRequest" to participantRequest,
                    "StationStatus" to participantResource.data?.stationStatus!!,
                    "StatusCode" to participantResource.data.statusCode)
...
//                        stationCheckDialogFragment.show(fragmentManager!!)
                        val statusCheckNewDialogFragment = FFQStatusCheckNewDialogFragment()
                        statusCheckNewDialogFragment.arguments = bundleOf("ParticipantRequest" to participantRequest,
                            "StationStatus" to participantResource.data.stationStatus,
                            "StatusCode" to participantResource.data.statusCode,
                            "Type" to "Cancelled",
                            "is_Cancelled" to true)
                        statusCheckNewDialogFragment.show(fragmentManager!!)
                    }
                    else if (participantResource.data.statusCode == 100)
                    {
                        val statusCheckNewDialogFragment = FFQStatusCheckNewDialogFragment()
                        statusCheckNewDialogFragment.arguments = bundleOf("ParticipantRequest" to participantRequest,
                            "StationStatus" to participantResource.data.stationStatus,
                            "StatusCode" to participantResource.data.statusCode,
                            "Type" to "Completed",
                            "is_Cancelled" to false)
                        statusCheckNewDialogFragment.show(fragmentManager!!)
                    }
                    else if (participantResource.data.statusCode == 10)
                    {
                        val statusCheckNewDialogFragment = FFQStatusCheckNewDialogFragment()
                        statusCheckNewDialogFragment.arguments = bundleOf("ParticipantRequest" to participantRequest,
                            "StationStatus" to participantResource.data.stationStatus,
                    ...
                    else if (participantResource.data.statusCode == 1)
                    {
                        val statusCheckDialogFragment = FFQStatusCheckDialogFragment()
                        statusCheckDialogFragment.arguments = bundleOf("ParticipantRequest" to participantRequest)
                        statusCheckDialogFragment.show(fragmentManager!!)
                    }
                }
