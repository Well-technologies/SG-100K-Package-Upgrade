package org.singapore.ghru.ui.hipwaist.hip
...
class HipFragment : Fragment(), Injectable {
    @Inject
    ...
    var binding by autoCleared<HipFragmentBinding>()
    ...
    @Inject
    lateinit var viewModel: HipViewModel
...
//        bodyMeasurementReading3 = BodyMeasurementValue.build()
        ...
        try {
            ...
            hipData = arguments?.getParcelable<HipWaistData>("HipData")!!
...
//        )
    ...
    override fun onCreateView(
        ...
        val dataBinding = DataBindingUtil.inflate<HipFragmentBinding>(
            inflater,
            R.layout.hip_fragment,
            container,
            false
        )
        binding = dataBinding
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        binding.measurement = hipData

        if (hipData != null)
        {
            binding.hipR1EditText.setText(hipData?.value?.get(0).toString())
            binding.hipR2EditText.setText(hipData?.value?.get(1).toString())
            binding.hipR3EditText.setText(hipData?.value?.get(2).toString())
        }
...
//            }else
            if (validateHipReading1(binding.hipR1EditText.text.toString()) && validateHipReading2(binding.hipR2EditText.text.toString()) && validateHipReading3(binding.hipR3EditText.text.toString()))
            {
                val hipList:ArrayList<Double> = arrayListOf()
                hipList.add(binding.hipR1EditText.text.toString().toDouble())
                hipList.add(binding.hipR2EditText.text.toString().toDouble())
                hipList.add(binding.hipR3EditText.text.toString().toDouble())

                if (validateDifference(hipList))
                {
                    val record = HipWaistData(
                        unit = "cm")

                    record.value = hipList

                    Log.d("HIP_FRAGMENT","DATA: " + record)

                    HipRecordTestRxBus.getInstance().post(record)
                    binding.root.hideKeyboard()
        ...
        editText.addTextChangedListener(object : TextWatcher {
            override fun afterTextChanged(p0: Editable?) {
                if(editText == binding.hipR1EditText) {
                    validateHipReading1(binding.hipR1EditText.text.toString())
                }

                if(editText == binding.hipR2EditText) {
                    validateHipReading2(binding.hipR2EditText.text.toString())
                }

                if(editText == binding.hipR3EditText) {
                    validateHipReading3(binding.hipR3EditText.text.toString())
                }
            }
    ...
    private fun validateDifference(hipData : ArrayList<Double>):Boolean
    {
        val intHipArray = hipData.map(Double::toDouble)
        val maxHip = intHipArray.max() ?: 0.0
        val minHip = intHipArray.min() ?: 0.0

        val hipDiff = maxHip - minHip

        Log.d("HIPFRAGMENT" , "DATA: " + hipDiff)
